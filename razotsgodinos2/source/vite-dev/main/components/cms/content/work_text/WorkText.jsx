...
class WorkText extends Component {
	...
	renderLangTab = lang => {
		...
		const work_Text = get(_langData, 'work_Text', '');

		return (
			<Fragment>
				<Field
					label="Apraksts"
					name={`${lang}_work_Text`}
					component={TextArea}
					value={work_Text}
				/>
			</Fragment>
	...
	render() {
		return (
			<Fragment>
				<EditContentForm name="work_text">
					{this.renderForm()}
				</EditContentForm>
			</Fragment>
		);
...
WorkText.propTypes = propTypes;

WorkText.defaultProps = defaultProps;

WorkText = WithUi(uiProps)(WorkText);
