...
class AboutHead extends Component {
	...
	renderLangTab = lang => {
		...
		const aboutHeader = get(_langData, 'aboutHeader', '');

		return (
			<Fragment>
				<Field
					label="Virsraksts"
					name={`${lang}_aboutHeader`}
					component={Input}
					value={aboutHeader}
				/>
			</Fragment>
	...
	render() {
		return (
			<Fragment>
				<EditContentForm name="about_head">
					{this.renderForm()}
				</EditContentForm>
			</Fragment>
		);
...
AboutHead.propTypes = propTypes;

AboutHead.defaultProps = defaultProps;

AboutHead = WithUi(uiProps)(AboutHead);
