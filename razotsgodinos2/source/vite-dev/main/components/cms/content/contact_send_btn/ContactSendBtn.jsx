...
class ContactSendBtn extends Component {
	constructor(props) {
		super(props);
	}
	...
	renderLangTab = lang => {
		...
		const contact_send_Btn = get(_langData, 'contact_send_Btn', '');

		return (
			<Fragment>
				<Field
					label="Pogas nosaukums"
					name={`${lang}_contact_send_Btn`}
					component={Input}
					value={contact_send_Btn}
				/>
			</Fragment>
		);
		//</editor-fold>
	...
	render() {
		return (
			<Fragment>
				<EditContentForm name="contact_send_btn">
					{this.renderForm()}
				</EditContentForm>
			</Fragment>
		);
...
ContactSendBtn.propTypes = propTypes;

ContactSendBtn.defaultProps = defaultProps;

ContactSendBtn = WithUi(uiProps)(ContactSendBtn);
