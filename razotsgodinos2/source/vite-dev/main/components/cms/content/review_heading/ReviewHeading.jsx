...
class ReviewHeading extends Component {
	constructor(props) {
		super(props);
	}
	...
	renderLangTab = lang => {
		...
		const review_head = get(_langData, 'review_head', '');

		return (
			<Fragment>
				<Field
					label="Nosaukums"
					name={`${lang}_review_head`}
					component={Input}
					value={review_head}
				/>
			</Fragment>
	...
	render() {
		return (
			<Fragment>
				<EditContentForm name="review_heading">
					{this.renderForm()}
				</EditContentForm>
			</Fragment>
		);
...
ReviewHeading.propTypes = propTypes;

ReviewHeading.defaultProps = defaultProps;

ReviewHeading = WithUi(uiProps)(ReviewHeading);
