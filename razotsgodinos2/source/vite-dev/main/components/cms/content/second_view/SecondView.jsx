import React, { PureComponent as Component, Fragment } from 'react';
import PropTypes from 'prop-types';

import WithUi from 'hoc/store/ui';
import { get, head } from 'lodash-es';

import EditContentForm from 'cms/content';
import Field from 'ui/form/field';
import Input from 'ui/inputs/input';
import LangsTab from 'ui/common/langs_tab';

const propTypes = {
	//from ui
	loading: PropTypes.bool,
	langData: PropTypes.object,
	langs: PropTypes.array,
};

const defaultProps = {
	//from ui
	loading: true,
};

const uiProps = ownProps => {
	return {
		CMSPopup: {
			loading: 'loading',
			langs: 'langs',
			content: {
				langData: 'langData',
			},
		},
	};
};

class SecondView extends Component {
	constructor(props) {
		super(props);
	}

	componentDidMount() {
		//<editor-fold defaultstate="collapsed" desc="componentDidMount">
		uiStore.update('CMSPopup', {
			title: 'Rediģēt',
		});
		//</editor-fold>
	}

	renderForm = () => {
		//<editor-fold defaultstate="collapsed" desc="renderForm">
		const { loading, langs  } = this.props;

		if (loading) {
			return null;
		}

		
		return (
			<Fragment>
				<LangsTab langs={langs} renderItem={this.renderLangTab} />
			</Fragment>
		);
		//</editor-fold>
	};

	renderLangTab = lang => {
		//<editor-fold defaultstate="collapsed" desc="renderLangTab">
		const { langData } = this.props;
		const _langData = get(langData, lang, {});

		const type = get(_langData, 'type', '');
		const area = get(_langData, 'area', '');
		const prices = get(_langData, 'prices', '');
		const price = get(_langData, 'price', '');
		const pvn = get(_langData, 'pvn', '');
		const note = get(_langData, 'note', '');
		const contacts = get(_langData, 'contacts', '');

		return (
			<Fragment>
				<Field
					label="(Projekta tips)"
					name={`${lang}_type`}
					component={Input}
					value={type}
				/>
				<Field
					label="(Platība)"
					name={`${lang}_area`}
					component={Input}
					value={area}
				/>
				<Field
					label="(Provizoriskās izmaksas)"
					name={`${lang}_prices`}
					component={Input}
					value={prices}
				/>
				<Field
					label="(Eur)"
					name={`${lang}_price`}
					component={Input}
					value={price}
				/>
				<Field
					label="(bez PVN)"
					name={`${lang}_pvn`}
					component={Input}
					value={pvn}
				/>
				<Field
					label="(Kalkulācija...)"
					name={`${lang}_note`}
					component={Input}
					value={note}
				/>
				<Field
					label="(Kontaktinformācija)"
					name={`${lang}_contacts`}
					component={Input}
					value={contacts}
				/>
			</Fragment>
		);
		//</editor-fold>
	};

	render() {
		return (
			<Fragment>
				<EditContentForm name="second_view">
					{this.renderForm()}
				</EditContentForm>
			</Fragment>
		);
	}
}

SecondView.propTypes = propTypes;

SecondView.defaultProps = defaultProps;

SecondView = WithUi(uiProps)(SecondView);

export default SecondView;
