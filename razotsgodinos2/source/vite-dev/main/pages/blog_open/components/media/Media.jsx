...
class Media extends Component {
	...
	render() {
		...
		const hasGallery = (blog?.gallery || []).length > 0;
		const hasVideo = (blog?.videos || []).length > 0;

		if (!hasGallery && !hasVideo) return null;

		const activeTab = this.getActiveTab(hasGallery, hasVideo);
		const firstVideo = blog?.videos?.[0];
		...
		return (
			<div className={styles.container}>
				<div className={styles.wrapper}>
					...
					{activeTab === 'gallery' && hasGallery && <Gallery blog={blog} />}
					{activeTab === 'video' && hasVideo && <Video video={firstVideo} />}

					<div className={styles.social_media}>
						<div className={styles.share}>
							<Editable edit={{ name: 'share_with' }}>
...
export default WithUi(uiProps)(Media);
